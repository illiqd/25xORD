...
const outputPath = './images/output/snaked.png';
...
async function main() {
  ...
  while (pixelArray.length > 0) {
    ...
    if (lastPixel && lastPixel.pixel.join(',') === currentPixel.pixel.join(',')) {
      const adjacents = getAdjacentPositions(lastPixel.x, lastPixel.y);
      position = adjacents.find(pos => isAvailable(pos.x, pos.y));
    }
